  ...
  useEffect(() => {
    ...
    const initAnimations = () => {
      ...
      const moveCursor = (e) => {
        ...
        // Interactive particles - strong repulsion effect from cursor
        ...
        particles.forEach((particle) => {
          ...
          // Much larger effect radius (400px) with stronger force
          if (distance < 400) {
            const force = (400 - distance) / 400;
            // Increased force multiplier from 0.3 to 1.2 for stronger effect
            const moveX = -deltaX * force * 1.2;
            const moveY = -deltaY * force * 1.2;
            ...
            gsap.to(particle, {
              ...
              duration: 0.2,
              ease: "power2.out",
              // Add scale effect for more drama
              scale: 1 + force * 0.5,
        ...
        // Interactive grid - strong wave effect
        ...
        if (grid) {
          ...
          const offsetX = (mouseX - gridCenterX) / gridCenterX;
          const offsetY = (mouseY - gridCenterY) / gridCenterY;

          // Increased multiplier from 5 to 15 for stronger grid movement
          gsap.to(grid, {
            backgroundPosition: `${50 + offsetX * 15}% ${50 + offsetY * 15}%`,
            duration: 0.3,
            ease: "power2.out",
          });
        }
      ...
      if (aboutTexts.length > 0) {
        ...
        // This makes paragraphs animate sequentially as one continuous flow
      ...
      gsap.utils.toArray(".section").forEach((section) => {
        ...
        if (sectionTitle) {
          const split = new SplitType(sectionTitle, { types: "chars" });
          gsap.set(split.chars, { opacity: 1, y: 0, rotateX: 0 });
          gsap.fromTo(
            split.chars,
